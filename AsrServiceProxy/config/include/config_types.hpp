...
#ifndef CONFIG_CONFIG_TYPES_HPP
#define CONFIG_CONFIG_TYPES_HPP
...
// _fix compatibility with gcc 4.7
#define BOOST_VARIANT_NO_FULL_RECURSIVE_VARIANT_SUPPORT
#include <boost/variant.hpp>

// _force boost file-system version 2 for later boost versions > 1.46
...
#define DECLARE_SHARED_PTR(_type) \
    class _type; \
    typedef std::shared_ptr<_type> _type##_ptr;
...
template <class _t>
struct config_hash : std::unary_function<_t, std::size_t> {
    inline std::size_t operator()(const _t& type) const {
        return type ? type->hash_code() : 0;
    }
};

template <class _t>
struct config_equals {
    inline bool operator()(const _t& first, const _t& second) const {
...
typedef std::unordered_set<PathPtr, config_hash<PathPtr>, config_equals<PathPtr>> SetPath;
...
typedef std::unordered_map<MemoKeyPtr, AbstractConfigValuePtr, config_hash<MemoKeyPtr>, config_equals<MemoKeyPtr>>
        MapMemoKeyAbstractConfigValue;
typedef std::unordered_map<AbstractConfigValuePtr, ResolveReplacerPtr, config_hash<AbstractConfigValuePtr>>
        MapResolveReplacer;
typedef std::unordered_map<PathPtr, ConfigVariant, config_hash<PathPtr>, config_equals<PathPtr>>
        MapPathVariant;
typedef std::unordered_map<PathPtr, MapAbstractConfigValue, config_hash<PathPtr>, config_equals<PathPtr>>
        MapPathMapAbstractConfigValue;
